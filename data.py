...
def get_wds_dataset(args, preprocess_img, is_train, epoch=0, floor=False, tokenizer=None):
    ...
    def process_sample(sample):
        ...
        return {"aug1": img1, "aug2": img2, "text": txt}  # Assume "text" is the label key
    pipeline.extend([
        wds.select(filter_no_caption_or_no_image),
        wds.decode("pilrgb", handler=log_and_continue),
        wds.rename(image="jpg;png;jpeg;webp", text="txt"),
        wds.map(process_sample),  # Apply the custom processing function
        wds.to_tuple("aug1", "aug2", "text" ),
        wds.batched(args.batch_size, partial=not is_train) # default drop_last=True
    ])
